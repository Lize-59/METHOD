...
y_oth = np.loadtxt('活性值全.txt')
y_mean = np.full((1,len(y_oth)), np.sum(y_oth)/len(y_oth))
############################################
file = open('测试集.txt','r')
line = file.read()
line_1 = line.replace('\n','')
file_2 = open('测试集.txt','r')
line_2 = file_2.readlines()
number = len(line_2)
data_1 = np.zeros((number,20))
non = np.zeros((number*3, 20))
...
for i in range(number):
    ...
    if line[i*4+1:i*4+3] in all_data:
        data_2[i,all_data.index(line[i*4+1:i*4+3])] = 1  
X_test = np.c_[X_,data_1,data_2]
#################################################################           
regr = svm.SVR(kernel ='rbf',degree = 3,gamma = 0.03125 ,coef0 = 0.0,
		    tol = 0.001,C = 64.0,epsilon = 0.00390625,shrinking = True,cache_size = 40,		
		    verbose = False,max_iter = -1)            
regr.fit(X, y)

y_pre = regr.predict(X_test)      
num = (1-np.sum((y_oth-y_pre)**2)/np.sum((y_oth-y_mean)**2))
